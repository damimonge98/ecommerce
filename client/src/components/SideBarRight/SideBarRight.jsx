...
export default function SideBarRight() {
  ...
  useEffect(() => {
    const moverLocalABack = async (state, userId) => {
      ...
      for (const localProduct of state.carrito.products) {
        const productBack = data.find((x) => x.productId == localProduct.id);
        if (productBack) {
          const result = clienteAxios.put(`orders/users/${userId}/cart`, {
            productId: localProduct.id,
            cantidad: productBack.cantidad + localProduct.cantidad,
          });
          console.log(result);
        } else {
          const result = clienteAxios.post(`orders/users/${userId}/cart`, {
            productId: localProduct.id,
    ...
    if (userData.isAuthenticated) {
      ...
      if (state && state.carrito.products.length > 0) {
